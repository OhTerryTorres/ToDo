...
class APIRequestService {
    ...
    func postRequest(task: Task, method: Method) {
        ...
        if let userCreated = task.userCreated {
            json[TaskPropertyKeys.userCreated.rawValue] = userCreated
        }
        if let userCompleted = task.userCompleted {
            json[TaskPropertyKeys.userCompleted.rawValue] = userCompleted
        }
        if let dateCreated = task.dateCreated {
            json[TaskPropertyKeys.dateCreated.rawValue] = MySQLDateFormatter.string(from: dateCreated as Date)
        }
        if let dateCompleted = task.dateCompleted {
            json[TaskPropertyKeys.dateCompleted.rawValue] = MySQLDateFormatter.string(from: dateCompleted as Date)
        }
        print("dateCompleted before serializing is \(json["dateCompleted"])")
        
        print("serializing outgoing JSON")
        guard let jsonData = try? JSONSerialization.data(withJSONObject: json) else {
            print("body JSONSerialization error")
            return
        }
        ...
        let dataTask = URLSession.shared.dataTask(with: request) { (data, response, error) in
            ...
            do {
                if let json = try JSONSerialization.jsonObject(with: data, options: .allowFragments) as? [[String: Any]] {
                    ...
                    self.responseHandler!.handleResponse(json: json)
                }
            } catch {
